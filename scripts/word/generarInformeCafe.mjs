...
sections.push(
  heading1('Auditoría del modelo financiero (mayo 2026)'),
  p('El modelo financiero original presentaba un valor presente neto cercano a los CLP 543 millones para la mejor ubicación, lo que tras revisión técnica resultó incompatible con la realidad operativa del retail food en Chile. Una cafetería pequeña (30-35 m²) con un combo único no genera, en condiciones razonables, retornos absolutos superiores a 5x la inversión inicial en cinco años. La presente versión del informe incorpora seis correcciones documentadas a continuación, que reducen el VAN base de la ubicación ganadora a un nivel coherente con benchmarks sectoriales (Achiga 2024, Procafé 2024) y con la tasa de fracaso observada en cafeterías independientes en Chile (50-55% antes de los tres años, según el estudio Achiga 2024).'),
  heading2('Correcciones aplicadas'),
  tableFromRows([
    ['Variable', 'Antes', 'Ahora', 'Justificación'],
    ['Costo variable / combo', 'CLP 1.100 fijo', 'CLP 1.300 a 1.700 según zona', 'Combos con producto envasado real: insumos café, leche, vasos, sleeves, tapas. 40-50% del ticket en lugar del 24% original.'],
    ['Costos fijos no laborales', 'CLP 350.000 / mes', 'CLP 850.000 / mes', 'Faltaban: marketing (CLP 200k), aseo y pest control (CLP 100k), software y contador externo (CLP 150k), seguros y provisión de mantenciones (CLP 150k), servicios (CLP 250k).'],
    ['Comisión tarjetas', 'No considerada', '2,8% sobre ingresos', 'Transbank o Getnet ~3,5% sobre el ~80% de pago electrónico observado en cafeterías chilenas.'],
    ['Crecimiento demanda anual', '5,0%', '2,5%', 'Sectorial cafetería sin reinversión; 5% requeriría marketing adicional o expansión de capacidad.'],
    ['Valor terminal', 'Gordon Growth (multiplicador 10,7x)', 'Múltiplo EBITDA 3,5x', 'Cafetería independiente no opera a perpetuidad sin reposición de equipos. Múltiplo de salida es estándar en M&A retail food Chile (3-5x EBITDA).'],
    ['Tasa de costo de capital', '11,5%', '14,0%', 'Retail food en Chile es riesgo medio-alto (alta tasa de fracaso). CAPM: Rf 5,5% + β 1,3 × ERP 6,5%.'],
  ]),
  caption('Tabla A.1 — Correcciones aplicadas al modelo financiero entre la versión inicial y la versión auditada. Fuentes: Achiga 2024, Procafé 2024, Damodaran 2024, BCN 2025.'),
  heading2('Impacto en los resultados'),
  p(`El conjunto de correcciones reduce el VAN de la ubicación ganadora desde CLP 543 millones a ${fmtM(GANADORA.base.van)}, una magnitud coherente con benchmarks sectoriales para cafeterías independientes en zonas premium de la Región Metropolitana. La TIR se ajusta desde 119% a ${fmtPct(GANADORA.base.tir)}, valor que sigue siendo atractivo respecto al costo de capital exigido pero ya no presenta el sesgo de "asset light" extremo del modelo original. El payback aumenta de 1,18 a ${fmtPb(GANADORA.base.payback)}, también dentro de rangos sectoriales razonables. Tras esta corrección, el ranking de localizaciones también cambia: solo una ubicación (El Golf) presenta veredicto de viabilidad económica positiva con resiliencia razonable; otras dos (Santiago Centro y Providencia) presentan VAN marginalmente positivo en el caso base pero no resisten el escenario pesimista; las cuatro restantes presentan VAN claramente negativo y se desestiman.`),
  pageBreak(),
);
...
sections.push(
  ...
  heading2('13.1 Resumen de indicadores por ubicación (modelo corregido)'),
  tableFromRows([
...
sections.push(
  ...
  p(`Sobre la base del análisis financiero, normativo, comercial y de localización presentado, el proyecto evaluado presenta factibilidad económica positiva en una sola de las siete localizaciones candidatas. La aplicación de un modelo financiero realista —con costos fijos a precio de mercado, comisión de tarjetas, valor terminal por múltiplo conservador de EBITDA y tasa de descuento ajustada al riesgo del retail food chileno (${fmtPct(TCC)})— filtra rigurosamente las zonas donde el binomio ticket-volumen no es suficiente para sostener la estructura de costos elevada de una cafetería compacta.`),
