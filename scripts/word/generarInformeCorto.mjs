...
sections.push(
  ...
  p(`Este informe evalúa la factibilidad económica de un proyecto de cafetería de formato compacto (30-35 m²) que comercializa un combo único cerrado: espresso preparado al momento más un alimento envasado del proveedor. Se evaluaron siete localizaciones candidatas en la Región Metropolitana de Santiago aplicando un modelo financiero diseñado para el retail food chileno: tasa de descuento de 14% por CAPM (β 1,3 y ERP 6,5% según Damodaran 2024), valor terminal por múltiplo EBITDA de ${MULT_EBITDA_TERMINAL}x, comisión de tarjetas ${fmtPct(COMISION_TARJETAS)} sobre ingresos, régimen tributario Pro PYME 14 D N° 3, y proyección de demanda como producto del flujo peatonal por tasa de captura por penalización por competencia por curva de ramp-up año a año.`),
